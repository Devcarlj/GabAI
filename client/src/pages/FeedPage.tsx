...
export const FeedPage: React.FC = () => {
  ...
  const handleMobileNavClick = (id: string) => {
    ...
    if (id === 'map') navigate('/');
    if (id === 'report') navigate('/report');
    if (id === 'user') navigate('/profile');
  };
    ...
    <div className="flex flex-col h-dvh bg-[var(--theme-bg)] text-slate-100 pb-[88px] justify-between">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 text-center">
        <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800 shadow-xl max-w-xs w-full">
          ...
          <h1 className="text-lg font-bold text-slate-100">Feed Page</h1>
      ...
      {/* Bottom Navigation */}
      <MobileNavBar activeId="feed" onItemClick={handleMobileNavClick} />
    </div>
  );
};