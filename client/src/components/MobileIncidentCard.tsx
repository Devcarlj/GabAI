...
const urgencyStyles = (urgency: string) => {
  switch (urgency) {
    case "CRITICAL":
      return {
        badge: "bg-red-950/60 border-red-500 text-red-400",
        icon: "bg-red-500/15 text-red-500 border-red-500/40",
      };
    case "HIGH":
      return {
        badge: "bg-orange-950/60 border-orange-500 text-orange-400",
        icon: "bg-orange-500/15 text-orange-500 border-orange-500/40",
      };
    case "MEDIUM":
      return {
        badge: "bg-amber-950/60 border-amber-400 text-amber-300",
        icon: "bg-amber-400/15 text-amber-400 border-amber-400/40",
      };
    default:
      return {
        badge: "bg-emerald-950/60 border-emerald-500 text-emerald-400",
        icon: "bg-emerald-500/15 text-emerald-500 border-emerald-500/40",
      };
  }
};
...
export const MobileIncidentCard: React.FC<MobileIncidentCardProps> = ({
  ...
  const title =
    summary?.split(/[.!?]/)[0]?.trim() || `${urgency} Incident Report`;
  ...
  const reportsLabel = String(
    (ticket as unknown as { reportsCount?: number }).reportsCount ?? 1,
  );
  const statusLabel =
    (ticket as unknown as { status?: string }).status ?? "Active";

  return (
    <div
      className="relative flex flex-col gap-3.5 pb-10"
    ...
    >
      ...
      <div
        ...
        aria-label="Swipe down to close incident details"
      >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 min-w-0">
          <div
            className={`shrink-0 w-11 h-11 rounded-xl border flex items-center justify-center ${styles.icon}`}
          >
            <Icon className="w-5 h-5" strokeWidth={2.25} />
          </div>
          <div className="min-w-0 pt-0.5">
            <h3 className="text-[15px] font-bold text-slate-100 leading-snug truncate">
              {title}
            </h3>
        ...
        <span
          className={`shrink-0 px-2 py-0.5 text-[10px] font-bold border rounded-md tracking-wide ${styles.badge}`}
        >
          {urgency}
        </span>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        ...
        <StatBlock
          label="Reports"
          value={reportsLabel}
          valueClass="text-[var(--theme-accent)]"
        />
        <StatBlock
          label="Status"
          value={statusLabel}
